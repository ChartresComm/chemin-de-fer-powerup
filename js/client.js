...
TrelloPowerUp.initialize({
  ...
  'card-badges': function (t, options) {
    return cdfCardPageBadges(t);
  },

  // Même information, mais affichée dans le détail de la carte ouverte (section
  // "Power-Ups" de la barre latérale) — plus facile à repérer que sur la face de
  // la carte dans le tableau.
...
function cdfCardPageBadges(t) {
  return t.card('id').then(function (card) {
    return Promise.all([cdfGetLayout(t), cdfGetConfig(t)]).then(function (res) {
      ...
      return [{ title: 'Chemin de fer', text: texts.join(' + '), color: 'blue' }];
    });
  });
}
